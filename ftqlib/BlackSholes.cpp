...

{
	...
	u[1][1] = u[0][1] + tau * (pow(sigma, 2) * pow(S_min + h, 2) * (u[0][2] - 2 * u[0][1] + u[0][0]) / 4 / pow(h, 2) + r * (S_min + h) * (u[0][2] - u[0][0]) / 2 / h - r * u[0][1]);
	u[1][N - 1] = u[0][N - 1] + tau * (pow(sigma, 2) * pow(S_min + (N - 1) * h, 2) * (u[0][N] - 2 * u[0][N - 1] + u[0][N - 2]) / 4 / pow(h, 2) + r * (S_min + (N - 1) * h) * (u[0][N] - u[0][N - 2]) / 2 / h - r * u[0][N - 1]);
	std::vector<double> a;
	a.resize(N);
	std::vector<double> b;
	b.resize(N);
	...
	for (int j = 2; j <= N - 2; j++)
	{
		...
		f[j] = tau * (pow(sigma, 2) * pow(S_min + j * h, 2) / 4 / pow(h, 2) - r * (S_min + j * h) / 2 / h) * u[0][j - 1] - tau * (pow(sigma, 2) * pow(S_min + j * h, 2) / 2 / pow(h, 2) + r) * u[0][j] + u[0][j] + tau * (pow(sigma, 2) * pow(S_min + j * h, 2) / 4 / pow(h, 2) + r * (S_min + j * h) / 2 / h) * u[0][j + 1];
	}
	c[N - 1] = 1;
	a[N - 1] = 0;
	f[N - 1] = u[1][N - 1];
	ThomasAlgorithm(u[1], a, b, c, f);
	u[1][0] = 2 * u[1][1] - u[1][2];
	u[1][N] = 2 * u[1][N - 1] - u[1][N - 2];
	//Step 3:calculate remaining time layers
	for (int k = 2; k <= M; k++)
	{
		u[k][1] = u[k - 1][1] + tau * (r * (S_min + h) * (u[k - 1][2] - u[k - 1][0]) / 2 / h - r * u[k - 1][1]);
		u[k][N - 1] = u[k - 1][N - 1] + tau * (r * (S_min + (N - 1) * h) * (u[k - 1][N] - u[k - 1][N - 2]) / 2 / h - r * u[k - 1][N - 1]);
		c[1] = 1;
		b[1] = 0;
		f[1] = u[k][1];
		for (int j = 2; j <= N - 2; j++)
		{
			b[j] = a[j] = tau * pow(sigma, 2) * pow(S_min + j * h, 2) / 4 / pow(h, 2);
			c[j] = 1 + tau * pow(sigma, 2) * pow(S_min + j * h, 2) / 2 / pow(h, 2);
			f[j] = tau * (pow(sigma, 2) * pow(S_min + j * h, 2) / 4 / pow(h, 2) - r * (S_min + j * h) / 2 / h) * u[k-1][j - 1] - tau * (pow(sigma, 2) * pow(S_min + j * h, 2) / 2 / pow(h, 2) + r) * u[k-1][j] + u[k-1][j] + tau * (pow(sigma, 2) * pow(S_min + j * h, 2) / 4 / pow(h, 2) + r * (S_min + j * h) / 2 / h) * u[k-1][j + 1];
		}
		c[N - 1] = 1;
		a[N - 1] = 0;
		f[N - 1] = u[k][N - 1];
		ThomasAlgorithm(u[k], a, b, c, f);
		u[k][0] = 2 * u[k][1] - u[k][2];
